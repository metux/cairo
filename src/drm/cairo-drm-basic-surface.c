				    ...
				    void **image_extra)
{
    ...
    const cairo_drm_device_t *drm_dev = _cairo_abstract_device_cast_drm(surface->drm.base.device);
...
drm_basic_surface_paint (void *abstract_surface,
		     cairo_operator_t		 op,
		     const cairo_pattern_t	*source,
		     const cairo_clip_t		*clip)
{
    return _cairo_surface_paint (drm_basic_surface_map_to_image (abstract_surface),
				 op, source, clip);
}

static cairo_int_status_t
drm_basic_surface_mask (void			*abstract_surface,
		    cairo_operator_t		 op,
		    const cairo_pattern_t	*source,
		    const cairo_pattern_t	*mask,
		    const cairo_clip_t		*clip)
{
    return _cairo_surface_mask (drm_basic_surface_map_to_image (abstract_surface),
				op, source, mask, clip);
}

static cairo_int_status_t
drm_basic_surface_stroke (void			*abstract_surface,
		      cairo_operator_t		 op,
		      const cairo_pattern_t	*source,
		      ...
		      const cairo_clip_t		*clip)
{
    return _cairo_surface_stroke (drm_basic_surface_map_to_image (abstract_surface),
				  op, source, path, stroke_style, ctm, ctm_inverse,
				  tolerance, antialias, clip);
}

static cairo_int_status_t
drm_basic_surface_fill (void			*abstract_surface,
		    cairo_operator_t		 op,
		    const cairo_pattern_t	*source,
		    const cairo_path_fixed_t	*path,
		    ...
		    const cairo_clip_t		*clip)
{
    return _cairo_surface_fill (drm_basic_surface_map_to_image (abstract_surface),
				op, source, path, fill_rule,
				tolerance, antialias, clip);
}

static cairo_int_status_t
drm_basic_surface_glyphs (void			*abstract_surface,
		      cairo_operator_t		 op,
		      const cairo_pattern_t	*source,
		      ...
		      const cairo_clip_t	*clip)
{
    return _cairo_surface_show_text_glyphs (drm_basic_surface_map_to_image (abstract_surface),
					    op, source,
					    NULL, 0,
					    glyphs, num_glyphs,
					    NULL, 0, 0,
...
static const cairo_surface_backend_t basic_surface_backend = {
    ...
    .acquire_source_image	= drm_basic_surface_acquire_source_image,
    .release_source_image	= drm_basic_surface_release_source_image,
    ...
    .paint			= drm_basic_surface_paint,
    .mask			= drm_basic_surface_mask,
    .stroke			= drm_basic_surface_stroke,
    .fill			= drm_basic_surface_fill,
    .show_glyphs		= drm_basic_surface_glyphs,
};
...
_cairo_drm_basic_device_create (int fd, dev_t dev, int vendor_id, int chip_id)
{
    ...
    device->base.surface.map_to_image = drm_basic_surface_map_to_image;
