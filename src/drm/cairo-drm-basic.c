...
/*
void *
_cairo_drm_basic_bo_map (const basic_device_t *device, intel_bo_t *bo)
{
...
/*
cairo_bool_t
intel_bo_is_inactive (const cairo_drm_device_t *device, cairo_drm_bo_t *bo)
{
...
/*
    ...
    bo = _cairo_drm_bo_cast_basic (_cairo_drm_bo_from_pool (&device->base));
    if (unlikely (bo == NULL)) {
	_cairo_error_throw (CAIRO_STATUS_NO_MEMORY);
	return bo;
    }
			...
			int dst_x, int dst_y)
{
	...
	dst = device->base.bo.map (&(device->base), &(bo->base));
	if (dst == NULL) {
	    if (a8 != buf)
		free (a8);
			     ...
			     cairo_scaled_glyph_t  *scaled_glyph)
{
    ...
    if (dst == NULL) {
	dst = device->base.bo.map (&(device->base), &(cache->buffer.bo->base));
	if (unlikely (dst == NULL))
	    return _cairo_error (CAIRO_STATUS_DEVICE_ERROR);
    }
