...
void
_cairo_drm_basic_bo_write(const cairo_drm_device_t *device,
		cairo_drm_bo_t *bo,
...
//    assert (bo->tiling == I915_TILING_NONE);
    ...
    assert (size+offset <= bo->size);

//    intel_bo_set_tiling (device, bo);
//    assert (bo->_tiling == I915_TILING_NONE);

// FIXME: we need to make sure the bo is mmap'ed
    assert (bo->mapped);

// FIXME: dont we have an generic ioctl for that ?
    memcpy (bo->mapped+offset, data, size);

//    bo->busy = FALSE;
}

void
_cairo_drm_basic_bo_read(const cairo_drm_device_t *device,
	       cairo_drm_bo_t *bo,
	       unsigned long offset,
	       unsigned long size,
	       void *data)
...
// FIXME: we ned to make sure the bo is mmap'ed
    assert (bo->mapped);

// FIXME: dont we have an generic ioctl for that ?
    memcpy (data, bo->mapped+offset, size);
//    bo->cpu = TRUE;
//    bo->busy = FALSE;
}
...
/*
void *
basic_bo_map (const basic_device_t *device, intel_bo_t *bo)
{
...
/*
cairo_bool_t
intel_bo_is_inactive (const basic_device_t *device, intel_bo_t *bo)
{
...
/*
cairo_bool_t
intel_bo_wait (const basic_device_t *device, const intel_bo_t *bo)
{
    struct drm_i915_gem_set_domain set_domain;
    int ret;
...
/*
cairo_bool_t
intel_bo_madvise (basic_device_t *device,
		  intel_bo_t *bo,
		  int advice)
{
...
/*
static void
intel_bo_set_real_size (basic_device_t *device,
			intel_bo_t *bo,
			size_t size)
{
...
basic_bo_t *
basic_bo_create (basic_device_t *device,
	         uint32_t max_size,
	         uint32_t real_size,
...
basic_bo_t *
basic_bo_create_for_name (basic_device_t *device, uint32_t name)
{
/*
...
/*
void
intel_bo_set_tiling (const basic_device_t *device,
	             intel_bo_t *bo)
{
...
cairo_surface_t *
intel_bo_get_image (const basic_device_t *device,
		    intel_bo_t *bo,
		    const cairo_drm_surface_t *surface)
{
	...
	src = devie->base.bo_map(&(device->base), &(bo->base));
...
_intel_bo_put_a1_image (basic_device_t *device,
			intel_bo_t *bo,
			...
			int dst_x, int dst_y)
{
	...
	dst = device->base.bo_map (&(device->base), &(bo->base));
	if (dst == NULL) {
	    if (a8 != buf)
		free (a8);
...
cairo_status_t
intel_bo_put_image (basic_device_t *device,
		    intel_bo_t *bo,
		    cairo_image_surface_t *src,
		    int src_x, int src_y,
		    int width, int height,
...
/*
static void
_intel_gradient_cache_fini (basic_device_t *device)
{
    unsigned int n;
...
/*
static void
_intel_glyph_cache_fini (basic_device_t *device, intel_buffer_cache_t *cache)
{
...
void
_cairo_drm_basic_device_fini (basic_device_t *device)
{
    int n;
/*
...
/*
void
intel_throttle (basic_device_t *device)
{
...
/*
void
intel_glyph_cache_unpin (basic_device_t *device)
{
    int n;
...
intel_glyph_cache_add_glyph (basic_device_t *device,
			     ...
			     cairo_scaled_glyph_t  *scaled_glyph)
{
    ...
    if (dst == NULL) {
	dst = device->base.bo_map (&(device->base), &(cache->buffer.bo->base));
	if (unlikely (dst == NULL))
	    return _cairo_error (CAIRO_STATUS_DEVICE_ERROR);
    }
...
/*
static cairo_status_t
intel_get_glyph_cache (basic_device_t *device,
...
/*
cairo_int_status_t
intel_get_glyph (basic_device_t *device,
		 cairo_scaled_font_t *scaled_font,
		 cairo_scaled_glyph_t *scaled_glyph)
{
...
/*
cairo_status_t
intel_snapshot_cache_insert (basic_device_t *device,
...
intel_surface_detach_snapshot (cairo_surface_t *abstract_surface)
{
	...
	device = (basic_device_t *) surface->drm.base.device;
...
void
intel_snapshot_cache_thaw (basic_device_t *device)
{
...
cairo_status_t
intel_gradient_render (basic_device_t *device,
